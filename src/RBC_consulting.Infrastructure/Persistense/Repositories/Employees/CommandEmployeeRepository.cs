...
using VO = RBC_consulting.Domain.EmployeeAggregate.ValueObjects;
    ...
    ILogger<CommandEmployeeRepository> logger) : ICommandEmployeeRepository
{
    ...
    public async Task<Result<EmployeeId>> AddAsync(Employee entity, CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure<EmployeeId>(Error.Failure("Employee.AddError", ex.Message));
        }
    }
    ...
    public async Task<Result> DeleteAsync(EmployeeId id, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "Failed to delete employee {EmployeeId}", id.Value);
            return Result.Failure(Error.Failure("Employee.DeleteError", ex.Message));
        }
    }
    ...
    public async Task<Result<Employee>> GetAsync(EmployeeId id, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var email = row.Email is null ? null : VO.Email.Create(row.Email).Value;
            var phone = row.Phone is null ? null : VO.Phone.Create(row.Phone).Value;
            var salary = row.Salary is null ? null : VO.Salary.Create(row.Salary.Value).Value;

            return Result.Success(Employee.Reconstruct(
                new EmployeeId(row.EmployeeID),
                VO.EmployeeFullName.Create(row.FullName).Value,
                VO.Position.Create(row.Position).Value,
                VO.Department.Create(row.Department).Value,
                row.HireDate,
                email,
                phone,
                salary,
        ...
        {
            logger.LogError(ex, "Failed to get employee {EmployeeId}", id.Value);
            return Result.Failure<Employee>(Error.Failure("Employee.GetError", ex.Message));
        }
    }
    ...
    public async Task<Result> UpdateAsync(Employee entity, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(ex, "Failed to update employee {EmployeeId}", entity.Id.Value);
            return Result.Failure(Error.Failure("Employee.UpdateError", ex.Message));
        }
    }
